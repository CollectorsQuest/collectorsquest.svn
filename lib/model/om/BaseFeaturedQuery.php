...
abstract class BaseFeaturedQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = FeaturedPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    Featured A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `FEATURED_TYPE_ID`, `FEATURED_MODEL`, `FEATURED_ID`, `TREE_LEFT`, `TREE_RIGHT`, `TREE_SCOPE`, `EBLOB`, `START_DATE`, `END_DATE`, `POSITION`, `IS_ACTIVE` FROM `featured` WHERE `ID` = :p0';
    try
    {
    ...
    if ($row = $stmt->fetch(PDO::FETCH_NUM))
    {
      $obj = new Featured();
      $obj->hydrate($row);
      FeaturedPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
