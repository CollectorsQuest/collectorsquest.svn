...
abstract class BaseVideoCollectionCategoryQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = VideoCollectionCategoryPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    VideoCollectionCategory A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `VIDEO_ID`, `COLLECTION_CATEGORY_ID`, `CREATED_AT` FROM `video_collection_category` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new VideoCollectionCategory();
      $obj->hydrate($row);
      VideoCollectionCategoryPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
