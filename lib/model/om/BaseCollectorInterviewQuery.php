...
abstract class BaseCollectorInterviewQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = CollectorInterviewPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    CollectorInterview A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `COLLECTOR_ID`, `COLLECTION_CATEGORY_ID`, `COLLECTION_ID`, `TITLE`, `CATCH_PHRASE`, `IS_ACTIVE`, `CREATED_AT` FROM `collector_interview` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new CollectorInterview();
      $obj->hydrate($row);
      CollectorInterviewPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
