...
abstract class BaseSessionStorageQuery extends ModelCriteria
{
     ...
     * Example usage:
     * <code>
     * $query->filterBySessionTime(1234); // WHERE session_time = 1234
     * $query->filterBySessionTime(array(12, 34)); // WHERE session_time IN (12, 34)
     * $query->filterBySessionTime(array('min' => 12)); // WHERE session_time > 12
     * </code>
     *
     * @param     mixed $sessionTime The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
    ...
    public function filterByCollector($collector, $comparison = null)
    {
        if ($collector instanceof Collector) {
            ...
            return $this
                ->useCollectorQuery()
                ->filterByPrimaryKeys($collector->getPrimaryKeys())
                ->endUse();
        } else {
