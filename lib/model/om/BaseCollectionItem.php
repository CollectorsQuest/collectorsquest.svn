  ...
  const PEER = 'CollectionItemPeer';
	...
	public function setCollectibleId($v)
	{
		...
		if ($this->collectible_id !== $v) {
			...
			$this->modifiedColumns[] = CollectionItemPeer::COLLECTIBLE_ID;
		}
	...
	public function setScore($v)
	{
		...
		if ($this->score !== $v || $this->isNew()) {
			$this->score = $v;
			$this->modifiedColumns[] = CollectionItemPeer::SCORE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->est_value = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->purchased_price = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			$this->currency = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->photo = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->score = ($row[$startcol + 10] !== null) ? (int) $row[$startcol + 10] : null;
			$this->is_for_sale = ($row[$startcol + 11] !== null) ? (boolean) $row[$startcol + 11] : null;
			$this->num_views = ($row[$startcol + 12] !== null) ? (int) $row[$startcol + 12] : null;
			...
			return $startcol + 16; // 16 = CollectionItemPeer::NUM_COLUMNS - CollectionItemPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		$result = array(
			...
			$keys[6] => $this->getEstValue(),
			$keys[7] => $this->getPurchasedPrice(),
			$keys[8] => $this->getCurrency(),
			$keys[9] => $this->getPhoto(),
			$keys[10] => $this->getScore(),
			$keys[11] => $this->getIsForSale(),
			$keys[12] => $this->getNumViews(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[6], $arr)) $this->setEstValue($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setPurchasedPrice($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setCurrency($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setPhoto($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setScore($arr[$keys[10]]);
		if (array_key_exists($keys[11], $arr)) $this->setIsForSale($arr[$keys[11]]);
		if (array_key_exists($keys[12], $arr)) $this->setNumViews($arr[$keys[12]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CollectionItemPeer::COLLECTIBLE_ID)) $criteria->add(CollectionItemPeer::COLLECTIBLE_ID, $this->collectible_id);
		...
		if ($this->isColumnModified(CollectionItemPeer::SCORE)) $criteria->add(CollectionItemPeer::SCORE, $this->score);
	...
	public function getCollectible(PropelPDO $con = null)
	{
		if ($this->aCollectible === null && ($this->collectible_id !== null)) {
			$this->aCollectible = CollectibleQuery::create()->findPk($this->collectible_id);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aCollectible->addCollectionItems($this);
			 */
		}
