...
abstract class BaseTriviaQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = TriviaPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    Trivia A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `CONTENT`, `CREATED_AT` FROM `trivia` WHERE `ID` = :p0';
    try
    {
    ...
    if ($row = $stmt->fetch(PDO::FETCH_NUM))
    {
      $obj = new Trivia();
      $obj->hydrate($row);
      TriviaPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
