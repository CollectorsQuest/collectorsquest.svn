...
abstract class BaseCollectionCategoryQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = CollectionCategoryPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    CollectionCategory A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `PARENT_ID`, `NAME`, `SLUG`, `SCORE` FROM `collection_category` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new CollectionCategory();
      $obj->hydrate($row);
      CollectionCategoryPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
