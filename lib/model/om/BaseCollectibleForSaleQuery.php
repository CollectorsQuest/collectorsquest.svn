...
abstract class BaseCollectibleForSaleQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = CollectibleForSalePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    CollectibleForSale A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `COLLECTIBLE_ID`, `PRICE`, `CONDITION`, `IS_PRICE_NEGOTIABLE`, `IS_SHIPPING_FREE`, `IS_SOLD`, `IS_READY`, `QUANTITY`, `CREATED_AT`, `UPDATED_AT` FROM `collectible_for_sale` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new CollectibleForSale();
      $obj->hydrate($row);
      CollectibleForSalePeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
