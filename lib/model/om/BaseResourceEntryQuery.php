...
abstract class BaseResourceEntryQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = ResourceEntryPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    ResourceEntry A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `CATEGORY_ID`, `TYPE`, `NAME`, `SLUG`, `DESCRIPTION`, `URL`, `RSS`, `THUMBNAIL`, `BLOGGER`, `EMAIL`, `CREATED_AT` FROM `resource_entry` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new ResourceEntry();
      $obj->hydrate($row);
      ResourceEntryPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
