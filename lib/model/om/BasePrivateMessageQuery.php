...
abstract class BasePrivateMessageQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = PrivateMessagePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    PrivateMessage A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `THREAD`, `SENDER`, `RECEIVER`, `SUBJECT`, `BODY`, `IS_RICH`, `IS_READ`, `IS_REPLIED`, `IS_FORWARDED`, `IS_MARKED`, `IS_DELETED`, `CREATED_AT` FROM `private_message` WHERE `ID` = :p0';
    try
    {
    ...
    if ($row = $stmt->fetch(PDO::FETCH_NUM))
    {
      ...
      $obj->hydrate($row);
      PrivateMessagePeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
