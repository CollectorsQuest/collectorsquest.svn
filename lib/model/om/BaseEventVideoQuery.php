...
abstract class BaseEventVideoQuery extends ModelCriteria
{
  ...
  {
    $sql = 'SELECT `ID`, `EVENT_ID`, `TITLE`, `DESCRIPTION`, `LENGHT`, `THUMB_SMALL`, `THUMB_LARGE`, `FILENAME`, `VIEWS`, `CREATED_AT` FROM `event_video` WHERE `ID` = :p0';
    try
    {
    ...
    if ($row = $stmt->fetch(PDO::FETCH_NUM))
    {
      $obj = new EventVideo();
      $obj->hydrate($row);
      EventVideoPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
