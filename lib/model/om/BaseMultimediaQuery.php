...
abstract class BaseMultimediaQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = MultimediaPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    Multimedia A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `MODEL`, `MODEL_ID`, `TYPE`, `NAME`, `MD5`, `COLORS`, `ORIENTATION`, `SOURCE`, `IS_PRIMARY`, `CREATED_AT`, `UPDATED_AT` FROM `multimedia` WHERE `ID` = :p0';
    try
    {
    ...
    if ($row = $stmt->fetch(PDO::FETCH_NUM))
    {
      $obj = new Multimedia();
      $obj->hydrate($row);
      MultimediaPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
  ...
  {
    return $this->addUsingAlias(MultimediaPeer::UPDATED_AT, time() - $nbDays * 24 * 60 * 60, Criteria::GREATER_EQUAL);
  }
  ...
  {
    return $this->addUsingAlias(MultimediaPeer::CREATED_AT, time() - $nbDays * 24 * 60 * 60, Criteria::GREATER_EQUAL);
  }
  ...
  {
    return $this->addDescendingOrderByColumn(MultimediaPeer::UPDATED_AT);
  }
  ...
  {
    return $this->addAscendingOrderByColumn(MultimediaPeer::UPDATED_AT);
  }
  ...
  {
    return $this->addDescendingOrderByColumn(MultimediaPeer::CREATED_AT);
  }
  ...
  {
    return $this->addAscendingOrderByColumn(MultimediaPeer::CREATED_AT);
  }
