...
abstract class BaseCollectibleOfferArchiveQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = CollectibleOfferArchivePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    CollectibleOfferArchive A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `COLLECTIBLE_ID`, `COLLECTIBLE_FOR_SALE_ID`, `COLLECTOR_ID`, `PRICE`, `STATUS`, `UPDATED_AT`, `CREATED_AT`, `ARCHIVED_AT` FROM `collectible_offer_archive` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new CollectibleOfferArchive();
      $obj->hydrate($row);
      CollectibleOfferArchivePeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
