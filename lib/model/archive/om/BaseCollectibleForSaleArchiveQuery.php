...
abstract class BaseCollectibleForSaleArchiveQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = CollectibleForSaleArchivePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    CollectibleForSaleArchive A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `COLLECTIBLE_ID`, `PRICE`, `CONDITION`, `IS_PRICE_NEGOTIABLE`, `IS_SHIPPING_FREE`, `IS_SOLD`, `IS_READY`, `QUANTITY`, `UPDATED_AT`, `CREATED_AT`, `ARCHIVED_AT` FROM `collectible_for_sale_archive` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new CollectibleForSaleArchive();
      $obj->hydrate($row);
      CollectibleForSaleArchivePeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
