...
abstract class BaseCommentArchiveQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = CommentArchivePeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    CommentArchive A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `DISQUS_ID`, `PARENT_ID`, `COLLECTION_ID`, `COLLECTIBLE_ID`, `COLLECTOR_ID`, `AUTHOR_NAME`, `AUTHOR_EMAIL`, `AUTHOR_URL`, `SUBJECT`, `BODY`, `IP_ADDRESS`, `CREATED_AT`, `ARCHIVED_AT` FROM `comment_archive` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new CommentArchive();
      $obj->hydrate($row);
      CommentArchivePeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
