...
abstract class BaseCollectionItemQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = CollectionItemPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    CollectionItem A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `COLLECTION_ID`, `NAME`, `SLUG`, `DESCRIPTION`, `EST_VALUE`, `PURCHASED_PRICE`, `CURRENCY`, `PHOTO`, `IS_FOR_SALE`, `NUM_VIEWS`, `POSITION`, `CREATED_AT`, `UPDATED_AT` FROM `collection_item` WHERE `ID` = :p0';
    try
    {
    ...
    if ($row = $stmt->fetch(PDO::FETCH_NUM))
    {
      ...
      $obj->hydrate($row);
      CollectionItemPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
