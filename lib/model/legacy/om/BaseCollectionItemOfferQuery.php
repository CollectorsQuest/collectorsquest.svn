...
abstract class BaseCollectionItemOfferQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = CollectionItemOfferPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    CollectionItemOffer A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `ITEM_ID`, `ITEM_FOR_SALE_ID`, `COLLECTOR_ID`, `PRICE`, `STATUS`, `CREATED_AT`, `UPDATED_AT` FROM `collection_item_offer` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new CollectionItemOffer();
      $obj->hydrate($row);
      CollectionItemOfferPeer::addInstanceToPool($obj, (string) $row[0]);
    }
    $stmt->closeCursor();
