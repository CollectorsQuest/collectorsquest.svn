...
abstract class BaseCollectionItemOffer extends BaseObject  implements Persistent
{
  ...
  protected function doInsert(PropelPDO $con)
  {
    ...
    if ($this->isColumnModified(CollectionItemOfferPeer::ID))
    {
      $modifiedColumns[':p' . $index++]  = '`ID`';
    }
    if ($this->isColumnModified(CollectionItemOfferPeer::ITEM_ID))
    {
      $modifiedColumns[':p' . $index++]  = '`ITEM_ID`';
    }
    if ($this->isColumnModified(CollectionItemOfferPeer::ITEM_FOR_SALE_ID))
    {
      $modifiedColumns[':p' . $index++]  = '`ITEM_FOR_SALE_ID`';
    }
    if ($this->isColumnModified(CollectionItemOfferPeer::COLLECTOR_ID))
    {
      $modifiedColumns[':p' . $index++]  = '`COLLECTOR_ID`';
    }
    if ($this->isColumnModified(CollectionItemOfferPeer::PRICE))
    {
      $modifiedColumns[':p' . $index++]  = '`PRICE`';
    }
    if ($this->isColumnModified(CollectionItemOfferPeer::STATUS))
    {
    ...
    if ($this->isColumnModified(CollectionItemOfferPeer::UPDATED_AT))
    {
      $modifiedColumns[':p' . $index++]  = '`UPDATED_AT`';
    }

    $sql = sprintf(
      'INSERT INTO `collection_item_offer` (%s) VALUES (%s)',
      ...
      foreach ($modifiedColumns as $identifier => $columnName)
      {
        switch ($columnName)
        {
          ...
          case '`ITEM_ID`':
            ...
            break;
          case '`ITEM_FOR_SALE_ID`':
            $stmt->bindValue($identifier, $this->item_for_sale_id, PDO::PARAM_INT);
            break;
          case '`COLLECTOR_ID`':
