 ...
 *
 * @param  string   $ad_slot   Which ad slot to load
...
function cq_ad_slot($slot, $width, $height, $delayed = false)
{
  if (SF_ENV != 'prod')
  {
    $image = 'frontend/iab/'. $width .'x'. $height .'.gif';
  ...
  {
    echo sprintf(
      '<div align="center"><iframe src="/ad_slot.php?slot=%1$s&rand=%2$s" id="ad_slot_%1$s" frameborder="0" border="0" cellspacing="0" scrolling="no" marginwidth="0" marginheight="0" style="border: 0; width: %3$dpx; height: %4$dpx; padding: 0; margin: 0;" width="%3$d" height="%4$d"></iframe></div>',
...
{
  if (SF_ENV != 'prod' || sfContext::getInstance()->getRequest()->isXmlHttpRequest()) return;
...
function cq_echo_javascripts()
{
  ...
  if (!empty($contents))
  {
    include_once sfConfig::get('sf_lib_dir').'/vendor/JavaScriptMinify.class.php';
